 ...
 */
package com.zhishinet.image.demo;
...
public class Demo {
    ...
    /*public static void main(String[] args) {
        ...
        handwritingOcr(imageClient, bucketName);
    ...
    private static void ocrGeneral(ImageClient imageClient, String bucketName) {
        String ret;
        // 1. url方式
        String imgUrl = "http://test1-1257242748.picsh.myqcloud.com/r1.png";
        String imgUrl2 = "http://test1-1257242748.picsh.myqcloud.com/4图片.JPG";
        System.out.println("====================================================");
        GeneralOcrRequest request = new GeneralOcrRequest(bucketName, imgUrl2);
     ...
     * 手写体OCR
     *//*
    private static void handwritingOcr(ImageClient imageClient, String bucketName) {
        String ret;
        String ret2;
        // 1. url方式
        String imgUrl = "http://test1-1257242748.picsh.myqcloud.com/r1.png";
        String imgUrl2 = "http://test1-1257242748.picsh.myqcloud.com/4图片.JPG";
        String imgUrlHen = "http://test1-1257242748.picsh.myqcloud.com/hen.png";
        String imgUrlShu = "http://test1-1257242748.picsh.myqcloud.com/shu.png";
        System.out.println("====================================================");
        GeneralOcrRequest request = new GeneralOcrRequest(bucketName, imgUrlShu);
        ret = imageClient.handwritingOcr(request);
        System.out.println("handwritingOcr:" + ret);

        *//*GeneralOcrRequest request2 = new GeneralOcrRequest(bucketName, imgUrlShu);
        ret2 = imageClient.handwritingOcr(request2);
        System.out.println("handwritingOcr:" + ret2);*//*
        ...
        //retJson = new JSONObject(retJson.get("Items").toString());

        //System.out.println(retJson.get("Items").toString());
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if (i > 0){
                ...
                if (Math.abs(list.get(i).getItemCoord().getY() - list.get(i-1).getItemCoord().getY()) < list.get(i-1).getItemCoord().getHeight()
                        && Math.abs(list.get(i).getItemCoord().getX() - list.get(i-1).getItemCoord().getX()) > list.get(i-1).getItemCoord().getWidth()){
                    list.get(i-1).setItemstring(list.get(i-1).getItemstring() + list.get(i).getItemstring());
                    list.remove(i);
                }
            }
        }
        //List<Items> itemsList = jsonArray.toJavaObject(jsonArray, List.class);
        List<String> correct = new ArrayList<>(10);
        machine,waiter,coach,train,charge,in change of,field,tell the time,key,ring,key ring,cable,cable car
        correct.add("machine");
        correct.add("waiter");
        correct.add("coach");
        correct.add("train");
        correct.add("charge");
        correct.add("in change of");
        correct.add("field");
        correct.add("tell the time");
        correct.add("key");
        correct.add("ring");
        correct.add("key ring");
        correct.add("cable");
        correct.add("cable car");


        for (int i = 0; i < list.size(); i++) {

            // 去打分
            score(list.get(i).getItemstring(),correct.get(i));
     ...
     * @param str2 正确答案
     */
    private static void score(String str1, String str2) {
        ...
        System.out.println("result:" + (((double) maxLength - (double) result2) / (double) maxLength));
